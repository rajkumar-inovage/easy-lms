...
class Coachings_model extends CI_Model {
	...
	public function edit_plan ($coaching_id=0, $plan_id=0) {		
		...
		list ($em, $ed, $ey) = explode ('/', $end_date);
		$end_date = mktime (0, 0, 0, $em, $ed, $ey);		
		
		$this->db->set ("doj", $start_date);
		$this->db->set ("doe", $end_date);
		$this->db->where ("id", $coaching_id);
		$this->db->update ("coachings");
	...
	public function set_coaching_id ($coaching_id=0) {
		// Get coaching details
		//$coaching = $this->get_coaching ($coaching_id);
		//$coaching_name = $coaching['coaching_name'];
		//$coaching_url = $coaching['website'];
		if ($coaching_id > 0) {
			//$this->session->set_userdata ('coaching_id', $coaching_id);			
		} 
		//$this->session->set_userdata ('coaching_url', $coaching_url);
		//$this->session->set_userdata ('coaching_name', $coaching_name);
	}
	...
	public function send_plan_enquiry ($coaching_id=0, $plan_id=0, $member_id=0) {
		...
		// If this query is already there
		$this->db->where ('coaching_id',$coaching_id);
		$this->db->where ('plan_id', $plan_id);
		$this->db->where ('member_id', $member_id);
		$sql = $this->db->get ('plan_purchase');
		if ($sql->num_rows() == 0 ) {
			$this->db->insert ('plan_purchase', $data);
		}
	}
	
	public function upgrade_plan ($coaching_id=0, $plan_id=0) {
		$this->db->set ('subscription_type', $plan_id);
		$this->db->where ('id', $coaching_id);
		$this->db->update ('coachings');
	}
	...
	public function upgrade_requests ($coaching_id=0, $plan_id=0) {
		$this->db->select ('C.coaching_name, SP.title, PP.*');
		$this->db->from ('plan_requests AS PP');
		$this->db->join ('coachings AS C', 'PP.coaching_id=C.id', 'left');
		$this->db->join ('subscription_plans AS SP', 'PP.plan_id=SP.id', 'left');
		if ($coaching_id > 0) {
			$this->db->where ('PP.coaching_id', $coaching_id);			
		}
		if ($plan_id > 0) {
			$this->db->where ('PP.plan_id', $plan_id);			
		}
