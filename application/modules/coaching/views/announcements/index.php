...
if (!empty($results)) {
	foreach ($results as $row) {
		...
		?>
		<div class="card d-flex flex-row mb-3 alert-primary">
			<div class="d-flex flex-grow-1 min-width-zero">
				<div
					...
					<p class="mb-0 text-muted text-small w-xs-100" style="width:5%;">To</p>
					<p class="mb-0 text-muted text-small w-15 w-xs-100"><?php echo date('d M, Y', $row['end_date']); ?></p>
					<div class="mb-1 w-15 mt-3 mt-md-0 d-flex d-lg-block text-left text-md-right">
						<a href="javascript:void(0)" class="badge badge-pill badge-outline-info mb-1" style="font-size:0.8rem;" data-toggle="tooltip" data-placement="left" title="Send Notification">
							<i class="simple-icon-bell"></i>
						</a>
						<a href="javascript:void(0)" class="badge badge-pill badge-outline-danger mb-1 ml-2" style="font-size:0.8rem;" data-toggle="tooltip" data-placement="left" title="Delete Notification" onclick="show_confirm ('<?php echo 'Are you sure want to delete this announcement?'; ?>','<?php echo site_url('coaching/announcement_action/delete/' . $coaching_id . '/' . $row['announcement_id']); ?>' )">
