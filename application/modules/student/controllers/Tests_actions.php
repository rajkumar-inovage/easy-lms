...
//		$this->form_validation->set_rules ('test_mode', 'Test Mode', 'required');
		if ( $this->form_validation->run () == true )  {
			...
			$redirect = 'tests/admin/manage/'.$coaching_id.'/'.$category_id.'/'.$id;
			$this->output->set_content_type("application/json");
	...
	public function set_method ($category_id=0, $test_id=0, $method=0) {
		...
		if ($method == TEST_ADDQ_QB) {
			redirect('tests/admin/add_test_questions/'.$category_id.'/'.$test_id);
		} elseif ($method == TEST_ADDQ_CREATE) {
			redirect('tests/admin/question_group_create/'.$category_id.'/'.$test_id);			 
		} elseif ($method == TEST_ADDQ_UPLOAD) {
			redirect('tests/admin/upload_test_questions/'.$category_id.'/'.$test_id);			 
		} else {
			redirect('tests/admin/select_method/'.$category_id.'/'.$test_id);
		}
	}
	
	public function preview_test ($category_id=0, $test_id=0, $lesson_id=0, $offset=0) {
		$this->question_db->preview_test ($test_id=0, $offset=0);
	}
	
	/* QUETION DB- LIST ALL QUESTIONS
	//	Function to list all questions in selected lesson
	*/		
	public function question_db ( $category_id=0, $test_id=0, $lesson_id=0, $cat_ids=0, $diff_ids=0, $exclude=0, $show_questions=0) {
		$results = $this->qb_model->getQuestions ($lesson_id, $cat_ids, $diff_ids, $exclude);
		$num_questions  = 0;		
		if ( ! empty ($results) ) {
			foreach ($results as $questions) {
				$num_questions = $num_questions + count ($questions);
			}
		}
		...
		if ( ! empty ($questions)) {
			$test_questions = count ($questions);
		}
		
		/* Back Link */
		$data['bc'] = array ('Manage'=>'tests/admin/manage/'.$category_id.'/'.$test_id);
		
		/* Module Buttons */
		$data['crumb_buttons'] = $this->crumb_buttons;
		
		/* Page Related Toolbar Buttons */
		$data['toolbar_buttons'] = array ();
		$data['toolbar_buttons'] = array ( 
				'buttons'=>array ( 
					anchor ('tests/admin/preview_test/'.$category_id.'/'.$test_id.'/'.$lesson_id, '<i class="fa fa-search"></i> Preview Test' ),
					anchor ('tests/admin/index/'.$category_id, '<i class="fa fa-puzzle-piece"></i> Tests' ),
				));
		$data['toolbar_buttons']['buttons'][] = '<a href="javascript:void(0)" onclick="javascript:show_confirm ( \'Finalize test at '.$testMarks['marks'].' marks?\', \''.site_url('tests/admin/finalise_test/'.$category_id.'/'.$test_id).'\')" ><i class="fa fa-star"></i>Publish Test</a>';
		...
		$this->load->view ("ajax/add_questions", $data);
	}
	...
	public function save_test_questions ($category_id=0, $test_id=0, $lesson_id=0, $cat_ids=0, $diff_ids=0, $exclude=0) {
		...
		if ($this->form_validation->run() == true) {
			...
			$this->output->set_content_type("application/json");
			$this->output->set_output(json_encode(array('status'=>true, 'message'=>'Questions added successfully.', 'redirect'=>site_url('tests/admin/add_test_questions/'.$category_id.'/'.$test_id.'/'.$lesson_id.'/'.$cat_ids.'/'.$diff_ids.'/'.$exclude) )));
		} else {
			$this->output->set_content_type("application/json");
			$this->output->set_output(json_encode(array('status'=>false, 'error'=>validation_errors() )));
		}
	...
	*/
	public function remove_questions ($category_id=0, $test_id=0) {
		...
		if ($this->form_validation->run () == true) {
			...
			$this->output->set_content_type("application/json");
			$this->output->set_output(json_encode(array('status'=>true, 'message'=>'Question(s) removed successfully from test.', 'redirect'=>site_url('tests/admin/preview_test/'.$category_id.'/'.$test_id) )));
		} else {
			$this->output->set_content_type("application/json");
	...
	public function remove_question ($category_id=0, $test_id=0, $id=0, $lesson_id=0, $cat_ids=0, $diff_ids=0, $exclude=0) {
		...
		redirect ('tests/admin/preview_test/'.$category_id.'/'.$test_id);
	}	
	...
	public function remove_added_question ($category_id=0, $test_id=0, $id=0, $lesson_id=0, $cat_ids=0, $diff_ids=0, $exclude=0) {
		$result = $this->tests_model->deleteTestQuestion ($test_id, $id);
		redirect ('tests/admin/add_questions/'.$category_id.'/'.$test_id.'/'.$lesson_id.'/'.$cat_ids.'/'.$diff_ids.'/'.$exclude);
	}
	
	// Reset Test	
	public function reset_test ($category_id=0, $test_id=0 ) {
		$this->tests_model->resetTest ($test_id);
		...
		redirect ('tests/admin/manage/'.$category_id.'/'.$test_id);
	}
	
	/* Finalise Test
	// 
	*/
	public function finalise_test ($category_id=0, $test_id) {
		$question_ids = $this->tests_model->getTestQuestions($test_id);
		$num_questions = 0;
		...
		if ($num_questions == 0) {
			...
			$result = $this->tests_model->finaliseTest ($test_id, $total_marks);
	...
	// 
	*/
	public function unfinalise_test ($category_id=0, $test_id) {
		$result = $this->tests_model->unfinaliseTest ($test_id);
	...
	*/
	...
	/*---=====---*/
	public function get_users ($category_id=0, $test_id=0, $role_id=USER_ROLE_STUDENT, $class_id=0, $enroled=0) {
		if ($enroled == ENROLED_IN_TEST) {
			$results = $this->tests_model->get_users_in_test ($test_id );
		} else if ($enroled == ARCHIVED_IN_TEST) {
			$results = $this->tests_model->get_archived_students ($test_id);			
		} else {
			$results = $this->tests_model->get_users_not_in_test ($test_id, $role_id, $class_id);			
		}
		$data['results'] 		= $results;
		$data['category_id'] 	= $category_id;
		...
		$this->load->view ('enrolment/enrolment', $data);
	}
	
	public function enrol_users ($category_id=0, $test_id=0, $role_id=0, $class_id=0, $type=0, $batch_id=0, $status='-1') {		
		
		$this->form_validation->set_rules('users[]', 'Users', 'required');
		if ( $this->form_validation->run () == true ) {
			$ids = $this->input->post ('users');
			foreach ($ids as $member_id) {
				$this->tests_model->enrol_member ($member_id, $test_id);
			}
			$this->output->set_content_type("application/json");
			$this->output->set_output(json_encode(array('status'=>true, 'message'=>'User(s) enroled in test', 'redirect'=>site_url('tests/admin/enrolments/'.$category_id.'/'.$test_id.'/'.$role_id.'/'.$class_id.'/'.$type.'/'.$batch_id.'/'.$status) )));
		} else {
			$this->output->set_content_type("application/json");
			$this->output->set_output(json_encode(array('status'=>false, 'error'=>validation_errors() )));
		}
	...
	public function unenrol_users ($category_id=0, $test_id=0, $role_id=0, $class_id=0, $type=0, $batch_id=0, $status='-1') {
			...
			if (! empty ($ids)) {				
				foreach ($ids as $id) {
					if ($actions == 'archive') {
						...
						$x = $this->unenrol_user ($category_id, $test_id, $role_id, $class_id, $type, $batch_id, $status, $id, $redirect=0);
					}
				}
				$this->output->set_content_type("application/json");
				$this->output->set_output(json_encode(array('status'=>true, 'message'=>'User(s) un-enroled from test', 'redirect'=>site_url('tests/admin/enrolments/'.$category_id.'/'.$test_id.'/'.$role_id.'/'.$class_id.'/'.$type.'/'.$batch_id.'/'.$status) )));
			} else {
				$this->output->set_content_type("application/json");
	...
	public function unenrol_user ($category_id=0, $test_id=0, $role_id=0, $class_id=0, $type=0, $batch_id=0, $status='-1', $member_id=0, $redirect=1) { 
		$this->tests_model->unenrol_member ($id, $test_id); 
		if ($redirect == 1) {
			$this->message->set ('User un-enroled from test', 'success', true);
			redirect ('tests/admin/enrolments/'.$category_id.'/'.$test_id.'/'.$role_id.'/'.$class_id.'/'.$type.'/'.$batch_id.'/'.$status);
		}
	}
	
		// Ajax validation question group
	public function validate_question_group_create ($category_id=0, $test_id=0, $question_id=0) {
		...
		if (($this->form_validation->run() == true))  {
			$id = $this->qb_model->save_group (0, $question_id);
			$this->message->set ("Question heading created. Now add questions to it", 'success', true);
			$this->output->set_content_type("application/json");
			$this->output->set_output(json_encode(array('status'=>true, 'message'=>'Question created successfully', 'redirect'=>site_url('tests/admin/question_create/'.$category_id.'/'.$test_id.'/'.$id) )));
		} else {
			$this->output->set_content_type("application/json");
			$this->output->set_output(json_encode(array('status'=>false, 'error'=>validation_errors() )));
		}
	...
	public function validate_question_create ($category_id=0, $test_id=0, $parent_id=0, $question_id=0) {
		...
		if (($this->form_validation->run() == true))  {
			$id = $this->qb_model->save_question (0, $parent_id, $question_id);
			$this->tests_model->saveQuestionsSimple($id, $test_id);
			$this->message->set ("Question created successfully ", 'success', true);
			$this->output->set_content_type("application/json");
			$this->output->set_output(json_encode(array('status'=>true, 'message'=>'Question created successfully', 'redirect'=>site_url('tests/admin/question_create/'.$category_id.'/'.$test_id.'/'.$parent_id) )));
		} else {
			$this->output->set_content_type("application/json");
			$this->output->set_output(json_encode(array('status'=>false, 'error'=>validation_errors() )));
		}
	...
	*/
	public function ajax_question_delete ( $category_id=0, $test_id=0, $question_id=0) {
		$this->tests_model->deleteTestQuestion ( $test_id, $question_id);	
		//$this->qb_model->delete_questions ($question_id);
		echo 0;
	}
	
	/**/
	public function subscribe_category ($category_id=0, $plan_id=0, $member_id=0) {
		$subscribe = $this->tests_model->subscribe_category ($category_id, $plan_id, $member_id); 
		echo true;
	}
	
	public function unsubscribe_category ($category_id=0, $plan_id=0, $member_id=0) {
		$subscribe = $this->tests_model->unsubscribe_category ($category_id, $member_id);
		echo true;
	}
	...
	/*****************************************/
	public function submit_test ($coaching_id, $member_id, $category_id, $test_id) {
		$questions  = $this->input->post('questions');
		...
		redirect ('student/tests/test_submitted/'.$coaching_id.'/'.$member_id.'/'.$category_id.'/'.$test_id.'/'.$attempt_id);
		//echo site_url('student/tests/test_submitted/'.$coaching_id.'/'.$member_id.'/'.$category_id.'/'.$test_id.'/'.$attempt_id);
    exit;
